...
def add_alt_store(alt_store):
    config = read_config()
    alt_stores = config.get('alternativeStores')
    alt_stores.append({
...
def update_alt_store(alt_store_to_update):
    config = read_config()
    for alt_store in config.get('alternativeStores') :
...
def delete_alt_store(store_name):
    config = read_config()
    alt_stores = config.get('alternativeStores')
    config['alternativeStores'] = exclude_by_name_filter(alt_stores, store_name)
    write_json(CONFIG_LOCATION, config)
...
def display_alt_stores():
    display_alt_store(read_config().get('alternativeStores'))

def find_alt_store_by_name(store_name):
    config = read_config()
    return next((store for store in config.get('alternativeStores') if store.get('storeName') == store_name), None)

def switch_to_alt_store(store_name):
    alt_store = find_alt_store_by_name(store_name)
    store_location = alt_store.get('storeLocation')
    set_config_value('currentStoreLocation', store_location)

def get_applied_alt_store_name():
    alt_store_name = None
    config = read_config()
    commands_store_location = config.get('currentStoreLocation')
    alt_stores = config.get('alternativeStores')
